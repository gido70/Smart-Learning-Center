...
(() => {
  ...
  const DIALECT_RULES = [
    [/إذا أردنا نعدل أو نعمل إيقاف للمجموعة أو حذف لهذه المجموعة/gu,'إذا أردنا تعديل المجموعة أو إيقافها أو حذفها'],
    [/\bوكمان\b/gu,'وأيضًا'],[/\bوبنعتقد\b/gu,'ونعتقد'],[/\bفيها هي\b/gu,'هي'],
    [/نموذجًا نموذج تدريبي/gu,'نموذجًا تدريبيًا'],[/المصادر الذي أنا سأضعها/gu,'المصادر التي سأضعها'],
    [/إذا أردنا نعدل أو نعمل ايقاف للمجموعة أو حذف لهذه المجموعة/gu,'إذا أردنا تعديل المجموعة أو إيقافها أو حذفها'],
    [/\bليس ضايفها\b/gu,'لم يضفها'],[/\bليس ضايفه\b/gu,'لم يضفه'],[/\bبحيث ان أنا ده\b/gu,''],
    [/\bهتلاقي\b/gu,'ستجد'],[/\bلما يكون\b/gu,'عندما يكون'],[/\bهحطها\b/gu,'سأضعها'],[/\bهحطه\b/gu,'سأضعه'],
    [/\bكمان\b/gu,'أيضًا'],[/\bبرض\b/gu,'أيضًا'],[/\bبنعتقد\b/gu,'نعتقد'],[/\bبس\b/gu,'فقط'],
    [/\bبالنسبه\b/gu,'بالنسبة'],[/\bمهمه\b/gu,'مهمة'],[/\bمنصه\b/gu,'منصة'],[/\bايقاف\b/gu,'إيقاف'],
    [/\bالاعضاء\b/gu,'الأعضاء'],[/\bاساسي\b/gu,'أساسي'],[/نموذج أنا يمكن اعتبره/gu,'يمكن اعتبار هذا نموذجًا'],
    [/إذا أردنا نعدل او نعمل ايقاف للمجموعة او حذف لهذه المجموعة/gu,'إذا أردنا تعديل المجموعة أو إيقافها أو حذفها'],
    [/\bلما اضغط\b/gu,'عند الضغط'],[/\bيمكن تطلبها\b/gu,'يمكن طلبها'],[/\bإذا كان حتى\b/gu,'حتى إذا كان'],
    [/ايه الادوات اللي/gu,'ما الأدوات التي'],[/\bايه\b/gu,'ما'],[/\bحد يعرف\b/gu,'هل يعرف أحد'],[/\bعندنا\b/gu,'لدينا'],
    [/\bعندي\b/gu,'لدي'],[/\bاو\b/gu,'أو'],[/\bادوات\b/gu,'أدوات'],[/\bاداة\b/gu,'أداة'],
    [/\bلو هو\b/gu,'إذا كان'],[/\bممكن\b/gu,'يمكن'],[/\bانا\b/gu,'أنا'],[/للمجموعة ده/gu,'لهذه المجموعة'],
    [/\bلو حبينا\b/gu,'إذا أردنا'],[/\bلو حبيت\b/gu,'إذا أردت'],[/\bممكن نعمل\b/gu,'يمكن تنفيذ'],[/\bممكن نستخدم\b/gu,'يمكن استخدام'],
    [/\bقبل كده\b/gu,'سابقًا'],[/\bبعد كده\b/gu,'بعد ذلك'],[/\bنحطها\b/gu,'نضعها'],[/\bنحطه\b/gu,'نضعه'],[/\bعلى طول\b/gu,'مباشرة'],
    [/\bالخاص بيه\b/gu,'الخاص به'],[/\bالخاص بيها\b/gu,'الخاص بها'],[/\bمش ضايفها\b/gu,'لم يضفها'],[/\bمش ضايفه\b/gu,'لم يضفه'],
    [/\bمحتاجين\b/gu,'نحتاج'],[/\bمحتاج\b/gu,'يحتاج'],[/\bعايشين\b/gu,'نعيش'],[/\bعشان\b/gu,'لكي'],[/\bعلشان\b/gu,'لكي'],
    [/\bالمجموعة ده\b/gu,'هذه المجموعة'],[/\bالموضوع ده\b/gu,'هذا الموضوع'],[/\bالأداة دي\b/gu,'هذه الأداة'],[/\bالحاجة دي\b/gu,'هذه المسألة'],
  ...
  function cleanTranscript(raw) {
    let text = String(raw || '')
      .replace(/\r/g, '\n')
      // بعض أدوات التفريغ تلصق حرفًا لاتينيًا بالرقم: 8g ثواني، 44g دقيقة.
      .replace(/(?<![\p{L}\p{N}])(?:و)?\d+\s*[a-z]?\s*(?:ساعة|ساعات|ساعتان|دقيقة|دقائق|ثانية|ثانيتين|ثوان|ثواني)(?:\s*و?\s*\d+\s*[a-z]?\s*(?:دقيقة|دقائق|ثانية|ثانيتين|ثوان|ثواني))*/giu, ' ')
      .replace(/(?:^|\s)(?:ثانية|ثانيتين|ثوان|ثواني)(?=\s)/gu, ' ')
      .replace(/(?:ساعة|ساعات|دقيقة|دقائق|ثانية|ثانيتين|ثوان|ثواني)(?=[\p{L}])/gu, ' ')
      .replace(/^\s*(?:\[)?\d{1,2}:\d{2}(?::\d{2})?(?:\])?\s*/gm, '')
      .replace(/(?<![\p{L}\p{N}])\d{1,2}:\d{2}(?::\d{2})?(?![\p{L}\p{N}])/gu, ' ')
      .replace(/(?:^|\s)(?:من\s+)?(?:ال)?ساعة\s*و?\s*\d+\s*(?:دقيقة|دقائق)(?:\s*و?\s*\d+\s*(?:ثانية|ثوان|ثواني))?/gu, ' ')
      .replace(/(?<![\p{L}\p{N}])\d+\s*(?:ساعة|ساعات|ساعتان)(?:\s*و?\s*\d+\s*(?:دقيقة|دقائق))?(?:\s*و?\s*\d+\s*(?:ثانية|ثوان|ثواني))?/gu, ' ')
      .replace(/(?<![\p{L}\p{N}])\d+\s*(?:دقيقة|دقائق)(?:\s*و?\s*\d+\s*(?:ثانية|ثوان|ثواني))?/gu, ' ')
      .replace(/(?<![\p{L}\p{N}])\d+\s*(?:ثانية|ثوان|ثواني)/gu, ' ')
      .replace(/\n{3,}/g, '\n\n')
      .replace(/[ \t]+/g, ' ')
      .trim();
    // تمريرات متتابعة لأن بعض العبارات تصبح قابلة للتصحيح بعد تحويل كلمة عامية داخلها.
    for(let pass=0;pass<3;pass++) DIALECT_RULES.forEach(([pattern,replacement])=>{
  ...
  async function summarizeNow() {
    ...
    button.disabled = true; status.textContent = 'جارٍ تنظيف النص واستخراج أهم الأفكار...';
    try {
      ...
      status.textContent = '✅ اكتملت الخلاصة وحُفظت داخل المحاضرة.';
    } catch (error) {
